...
const MessageDoctor = () => {
    ...
    const messageEndRef = useRef(); // Rendering end of the page
    ...
    const handleSubmit = async(evt)=>{
        ...
        try{
          ...
          const response = await fetch(`${backend_URL}/api/messages/patientSendMessage/${doctorId}`,fetchOptions);
    ...
    const fetchMessages = async ()=>{
      try{
        ...
        const response = await fetch(`${backend_URL}/api/messages/patientGetMessages/${doctorId}`,fetchOptions);
        const data = await response.json();
  ...
  return (
    <div>
        ...
        {/** Header */}
    <div className="p-2.5 border-b border-base-300" style={{width:"100%",height:"50px",marginTop:"20px", paddingBottom:'20px'}}>
      <div className="flex items-center justify-end" style={{}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",width:"100%"}} >
          ...
          <div className="avatar">
            <div className="size-10 rounded-full relative" style={{backgroundColor:"green"}}>
              <img src={ messages[0]?.doctorId.profilePic || "/user.png"} alt="Doctor" style={{borderRadius:"50%", height:"100%", width:"100%"}} />
            </div>
          </div>

          {/* User info */}
          <div>
            <h3 className="text-xl font-bold"> { messages[0]?.doctorId.fullName || "Doctor"} </h3>
          </div>
...
*/}
        {
          messages.map((message,ind)=>{
            return (
              <div className="hold-message" style={message.isSenderPatient ? {justifyContent:"flex-end"} : {justifyContent:'flex-start'} } key={message._id} >
    ...
    <div className="message-chat-inputs">
      {
        ...
        <div className="mb-3 flex items-center gap-2" style={{position:"absolute",bottom:"70px",left:"50px"}}>
          <div className="relative">
            <img src={ formData.image || "/user.png"} alt="Preview" className="w-20 h-20 object-cover rounded-md border border-zinc-700" />
      ...
      <form onSubmit={handleSubmit}>
        ...
        <label htmlFor="image-button">
            ...
            <button id="image-button" type='button' onClick={()=>imageHandle.current?.click() } style={{backgroundColor:'green',color:'white'}} > <Image size={20} />  </button>
        </label>
        {
          loading ?
